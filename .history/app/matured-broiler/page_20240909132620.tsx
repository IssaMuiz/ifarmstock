...
const MaturedBroiler = () => {
  ...
  useEffect(() => {
    ...
    const priceFilter = activeFilter.filter((filter) =>
      filter.includes("price")
    );

    if (priceFilter.length > 0) {
      filtered = filtered.filter((product) => {
        if (priceFilter.includes("price-low")) {
          return product.Price <= 50;
        }

        if (priceFilter.includes("price-high")) {
          return product.Price > 50;
        }
        return true;
      });
    }
    const brandFilter = activeFilter.filter((filter) =>
      filter.includes("brand")
    );
    if (brandFilter.length > 0) {
      filtered = filtered.filter((product) =>
        brandFilter.includes(`brand-${product.brand.toLowerCase()}`)
      );
    }

    const ratingFilter = activeFilter.filter((filter) =>
      filter.includes("rating")
    );

    if (ratingFilter.length > 0) {
      filtered = filtered.filter((product) => {
        const selectedRating = Math.max(
          ...ratingFilter.map((filter) => parseInt(filter.split("-")[1]))
        );
  ...
  const handleFilterChange = (selectedFilter: string) => {
    setActiveFilter((prevFilter) =>
      prevFilter.includes(selectedFilter)
        ? prevFilter.filter((filter) => filter !== selectedFilter)

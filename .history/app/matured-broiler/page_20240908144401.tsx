...
const MaturedBroiler = () => {
  ...
  return (
    <section className="flex gap-5 tablet:mx-8 mt-20 h-auto">
      <Filter
      ...
      {products.length === 0 ? (
        ...
        <div className="Broiler-cards h-auto bg-white p-3 w-full rounded-md items-center ">
          ...
          <div className="bg-white h-auto flex flex-col items-center w-auto tablet:p-4 mobile:p-2">
            <ul className="grid desktop:grid-cols-3 tablet:grid-cols-2 mobile:grid-cols-1 p-4">
              {products.map((product) => (
                <li
                  key={product.id}
                  className="flex shadow-md hover:shadow-xl hover:scale-[1.01] hover:transition-all hover:duration-300 hover:ease-in-out flex-col tablet:min-w-[200px] mobile:min-w-[120px] tablet:p-7 tablet:gap-3 mobile:p-1 mobile:gap-1 rounded-md cursor-pointer"
                >
                  <Image
                    src={product.image}
                    alt="poultry"

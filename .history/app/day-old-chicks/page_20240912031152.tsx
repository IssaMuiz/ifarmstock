...
import dayOldChicks from "@/models/dayOldChicks.json";
import React, { useEffect, useState } from "react";
import Sorting from "@/components/sorting";
import Filter from "@/components/filter";
import Pagination from "@/components/pagination";

type ProductWithWeight = {
  id: number;
  image: string;
  name: string;
  Price: number;
...
type ProductType = ProductWithWeight | ProductWithOUtWeight;

type PriceType = [number, number] | null;
type weightType = [number, number] | null;

const DayOldChicks = () => {
  const [products, setProducts] = useState<ProductType[]>(dayOldChicks);
      ...
      if (weightRange) {
        filtered = filtered.filter((product) => {
          if ("weight" in product) {
            return (
              product.weight >= weightRange[0] &&
              product.weight <= weightRange[1]
            );
          }
          return true;
        });
      ...
      {loading ? (
        ...
        <div>
          <div className="text-center h-20 mobile:block tablet:hidden text-2xl mt-8 fixed top-16 left-0 z-10 right-0 text-black bg-white">
            <p className="mt-6">Day Old Chicks</p>

...
const Sidenav = () => {
  ...
  return (
    <nav
      className={`fixed overflow-scroll overscroll-x-none scroll z-20 left-0 top-0 bottom-0 w-[300px] transform transition-transform duration-300  bg-gray-100 ${
                  ...
                  {category.dropdownlink.map((dropdown, index) => (
                    <div className="hover:bg-gray-300" key={index}>
                      <Link href={dropdown.link}>
                        <div className="p-3 text-xl ">{dropdown.name}</div>
                      </Link>
                    </div>
                  ))}
                </div>

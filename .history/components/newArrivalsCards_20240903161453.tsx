...
const NewArrivalsCards = () => {
  ...
  return (
    <div className="bg-white h-auto w-auto tablet:p-4 mobile:p-1">
      <div className="grid tablet:grid-rows-2 desktop:grid-cols-5 tablet:grid-cols-3 mobile:grid-cols-2 w-auto tablet:gap-5 mobile:gap-2">
        {sliceNewArrivals.map((product, index) => (
          <div
            key={index}
            className="flex shadow-md hover:shadow-xl hover:scale-[1.01] hover:transition-all hover:duration-300 hover:ease-in-out flex-col tablet:p-7 tablet:gap-3 mobile:p-2 mobile:gap-1 rounded-md cursor-pointer"
          >
            <Image
              src={product.image}
              alt="poultry"

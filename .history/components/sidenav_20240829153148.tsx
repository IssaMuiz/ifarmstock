  ...
  const [categories, setCategories] = useState([
    {
      ...
      name: "Broiler",
      expanded: false,
      arrowIcon: "/assets/icons/right-single-arrow.png",
      dropdownlink: [
      ...
      <div>
        {categories.map((category) => (
          <div onClick={() => handleDropdown(category.id)} key={category.id}>
            <div className="text-xl flex justify-between font-semibold bg-gray-200 mt-5 p-2 cursor-pointer">
              <div>{category.name}</div>

              <div>{category.arrowIcon}</div>
            </div>

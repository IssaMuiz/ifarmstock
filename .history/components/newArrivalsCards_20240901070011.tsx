    ...
    <div className="bg-white h-auto w-auto p-4">
      <div className="grid grid-rows-2 grid-cols-10 overflow-x-auto h-72 w-[2000px]  gap-5">
        {newArrivalsCards.map((product, index) => (
          <div
            key={index}
            className="flex shadow-md h-64 w-auto hover:shadow-xl hover:scale-[1.01] hover:transition-all hover:duration-300 hover:ease-in-out flex-col p-7 gap-3 rounded-md cursor-pointer"
          >
            <Image
              src={product.image}
              alt="poultry"

...
const Navbar = () => {
  ...
  const totalQuantity = useSelector((state: RootState) => state.cart.cart);
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      setCartCount(totalQuantity.reduce((acc, item) => acc + item.quantity, 0));
    }
  }, [totalQuantity]);
  ...
  return (
    <nav>
      <div className="Tablet-nav mobile:hidden tablet:flex fixed bg-black text w-full h-16 flex items-center justify-between top-0 left-0 right-0 px-10 z-10">
        ...
        <Link
          href="/"
          className="Leftnav text text-3xl font-semibold cursor-pointer ml-5"
        >
          iFarmstock
        </Link>
        <div className="Middlenav flex flex-1 mx-5 max-w-[700px] items-center">
          ...
          />
          <div className="h-[37px] w-10 bg-green-600 pt-2 pl-2 rounded-r-xl cursor-pointer">
            <Image
              className=""
        ...
        <div className="Rightnav flex items-center justify-between gap-3">
          ...
          <Link
            href="/Cart"
            className="flex relative cursor-pointer p-3 hover:bg-[green] rounded-md"
          >
            <Image
      ...
      <div className="Mobile-nav mobile:flex flex-col tablet:hidden fixed top-0 left-0 right-0 h-24 w-full items-center py-3 gap-2 bg-black text z-10 px-3">
        <div className=" flex items-center justify-between w-full">
          <div className="flex gap-3 items-center">
            ...
            <p className="text-lg font-semibold">iFarmstock</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Link
              href="/Cart"
              className="flex relative cursor-pointer rounded-md"
            >
              <Image
                src="/assets/icons/cart.png"
                alt="cart-icon"
        ...
        <div className="w-full">
          <div className="flex items-center w-full">
            <div className="flex-1">
              <input
                className=" px-5 py-2 rounded-l-md text-black outline-nonem w-full"
                type="text"
                placeholder="Search for products, services and categories"
              />
            </div>
